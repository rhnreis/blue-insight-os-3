 ...
 */
function parseAbertura(dataAbertura: string | number | Date | null | undefined, horaAbertura?: string | number | null | undefined): Date | null {
  try {
...
export interface RecallAnalysis {
  recallsByClient: Map<string, RecallResult[]>; // key: COD_CLIENTE-COD_SERVICO_CLIENTE
  recallOrderIds: Set<number>; // Set of COD_SUPORTE that are recalls
  recallPairs: RecallResult[]; // All recall pairs for analysis
}

/**
 * Interface for date filter
 */
...
export function computeRecalls(orders: ServiceOrder[], dateFilter: DateFilter): RecallAnalysis {
  ...
  const recallPairs: RecallResult[] = [];
  ...
  const orderGroups = new Map<string, ServiceOrder[]>();
  
  for (const order of orders) {
    // Skip orders with missing essential data
    if (!order.COD_CLIENTE || !order.CATEGORIA) continue;
  ...
  for (const [groupKey, groupOrders] of orderGroups) {
    ...
    if (ordersWithDates.length < 2) continue;
    ...
    for (let i = 0; i < ordersWithDates.length; i++) {
      ...
      if (!SUBSEQUENTES.has(currentOrder.normalizedCategory)) continue;

      // Check if current order falls within the date filter (for recall candidates)
      const currentDate = currentOrder.aberturaDate!;
      let isInDateRange = true;
      if (dateFilter.dataInicio || dateFilter.dataFim) {
        if (dateFilter.dataInicio && currentDate < dateFilter.dataInicio) isInDateRange = false;
        if (dateFilter.dataFim && currentDate > dateFilter.dataFim) isInDateRange = false;
      }
      
      if (!isInDateRange) continue;

      // Look for most recent previous GATILHO within window
      let foundTrigger: typeof ordersWithDates[0] | null = null;
